...
const int kTraceCount = 5;          // stacked trace copies         (1 – 5)
...
const vec3 kColorCool = vec3(1.0, 1.0, 1.0);
const vec3 kColorWarm = vec3(0.2627, 0.2627, 0.2627);
...
const float kGlitchRate = 0.0;    // probability per 8 Hz tick    (0.0 – 1.0)
...
const float kDropoutRate = 0.0;    // probability per 3 Hz tick    (0.0 – 1.0)
...
void main()
{
    ...
    vec3 bg = vec3(texture2D(u_texture, vec2(sx + chroma, uv.y)).r, texture2D(u_texture, vec2(sx, uv.y)).g,
                   texture2D(u_texture, vec2(sx - chroma, uv.y)).b);
    float bg_a = texture2D(u_texture, uv).a;

    // ---- Film grain ------------------------------------------------
    float grain =
        (hash2(uv + fract(vec2(u_time * 7.31, u_time * 3.17))) * 2.0 - 1.0) * (kGrainQuiet + drive * kGrainLoud);
    bg += grain;
