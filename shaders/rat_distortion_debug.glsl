...
float triangle_shape(vec2 point, float height, float half_width)
{
    ...
    return step(0.0, point.y) * step(point.y, height) * step(abs(point.x), width_at_y);
}
...
void main()
{
    ...
    vec3 distorted_color = vec3(texture2D(u_texture, clamp(distorted_uv + vec2(chroma, 0.0), 0.0, 1.0)).r,
    ...
    vec3 final_color = distorted_color;
    final_color += alien_tint * alien_growth * (0.45 + 0.55 * dominant + edge_strength * 0.6);
    final_color = mix(final_color, alien_tint, alien_growth * 0.35);
    final_color = mix(base.rgb, final_color, 0.35 + edge_strength * 0.65);

    float distortion_strength = clamp(length(distortion / max(texel, vec2(0.0001))) * 0.12, 0.0, 1.0);
    vec3 source_view = base.rgb;
    vec3 edge_view =
        mix(base.rgb * 0.12, vec3(0.5 + 0.5 * edge_direction.x, 0.5 + 0.5 * edge_direction.y, edge_strength), 0.85);
    edge_view += vec3(0.50, 0.08, 0.02) * edge_band;
    edge_view += vec3(0.0, 0.22, 0.35) * distortion_strength;

    vec3 thorn_view = base.rgb * 0.08;
    thorn_view += vec3(growth_shape * 0.45, edge_band * 0.20, pulse_gate * growth_window * 0.08);
    thorn_view += alien_tint * alien_growth * 1.15;
    thorn_view += vec3(root_glow * 0.85);

    vec3 output_color = final_color;
    if (uv.x < 0.25)
    {
        output_color = source_view;
    }
    else if (uv.x < 0.50)
    {
        output_color = edge_view;
    }
    else if (uv.x < 0.75)
    {
        output_color = thorn_view;
    }

    float divider = max(max(1.0 - step(0.0025, abs(uv.x - 0.25)), 1.0 - step(0.0025, abs(uv.x - 0.50))),
                        1.0 - step(0.0025, abs(uv.x - 0.75)));
    output_color = mix(output_color, vec3(1.0), divider * 0.9);

    gl_FragColor = vec4(clamp(output_color, 0.0, 1.0), 1.0);
}